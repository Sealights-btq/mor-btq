...
pipeline {
   agent {
    ...
    environment {
        SL_TOKEN = (sh(returnStdout: true, script:"aws secretsmanager get-secret-value --region eu-west-1 --secret-id 'btq/mor_token' | jq -r '.SecretString' | jq -r '.mor_token'" )).trim()
            ...
            steps {
                script {
                    ...
                    IP = "${IDENTIFIER}"
                            stage("Updating Helm") {
                                sh script: """
                                    aws secretsmanager get-secret-value --region eu-west-1 --secret-id 'btq/mor_key_pair' | jq -r '.SecretString' | jq -r '.mor_key_pair' > key.pem
                                    chmod 0400 key.pem
